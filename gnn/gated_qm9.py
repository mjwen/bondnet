...
def parse_args():
    ...
    if len(args.gated_hidden_size) == 1:
        args.gated_hidden_size = args.gated_hidden_size * args.num_gat_layers
    else:
    ...
    #    args.gated_hidden_size = [val * 2 ** i for i in range(args.num_gat_layers)]
    # else:
    #    assert len(args.gated_hidden_size) == args.num_gat_layers, (
    #        "length of `gat-hidden-size` should be equal to `num-gat-layers`, but got "
    #        "{} and {}.".format(args.gated_hidden_size, args.num_gat_layers)
    #    )
