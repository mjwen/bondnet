...
def read_smiles_csv(filename):
    ...
    for rxn in df.itertuples():
        ...
        if num_columns == 4:  # charges not provided
            ...
            idx, reactant, fragment1, fragment2, charge_r, charge_fg1, charge_fg2 = rxn
            if pd.isna(fragment2) or pd.isna(charge_fg2):
                fragment2 = None

        r = get_mol(molecule_smiles_and_charge, molecules, reactant, int(charge_r))
        p1 = get_mol(molecule_smiles_and_charge, molecules, fragment1, int(charge_fg1))
...
def convert_smiles_csv(
    ...
    reactions = read_smiles_csv(filename)
